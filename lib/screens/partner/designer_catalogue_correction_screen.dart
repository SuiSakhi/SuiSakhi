...
class DesignerCatalogueCorrectionScreen extends StatefulWidget {
  const DesignerCatalogueCorrectionScreen({
    super.key,
  ...
  @override
  State<DesignerCatalogueCorrectionScreen> createState() =>
      _DesignerCatalogueCorrectionScreenState();
}

class _DesignerCatalogueCorrectionScreenState
    extends State<DesignerCatalogueCorrectionScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (design.ownerType != CatalogueDesignOwnerType.designer ||
          design.ownerAccountId != access.accountId ||
          design.ownerProfileId != access.profileId) {
        throw StateError(
          'This Design does not belong to the selected Designer profile.',
        );
      }
      ...
      _price.text = design.commercial.designerExpectedPrice?.toString() ?? '';
  ...
  Future<void> _resubmit() async {
    ...
    await _perform(() async {
      await CatalogueCorrectionService.resubmitDesignerCorrection(
        designId: widget.designId,
        draftVersionId: _draft!.draftVersion.versionId,
        title: _title.text,
        ...
        designerExpectedPrice: double.tryParse(_price.text.trim()),
        views: _views,
      );
      if (!mounted) {
        return;
