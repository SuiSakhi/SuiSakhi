...
import '../../core/metadata/printing_partner_capability_metadata.dart';
...
class PrintingApplicationScreen extends StatefulWidget {
  const PrintingApplicationScreen({super.key});
  @override
  State<PrintingApplicationScreen> createState() => _PrintingApplicationScreenState();
}

class _PrintingApplicationScreenState extends State<PrintingApplicationScreen> {
  ...
  final _averageTurnaroundHours = TextEditingController();
  ...
  bool _expressOrdersAvailable = false;
  ...
  @override
  void dispose() {
    for (final controller in [_contact, _business, _mobile, _email, _serviceArea, _teamSize, _normalCapacity, _peakCapacity, _notes, _minimumOrderQuantity, _averageTurnaroundHours]) { controller.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _initialize() async {
    try {
      ...
      if (user == null || phone == null || phone.isEmpty) { throw StateError('Please sign in again before starting a Printing Partner application.'); }
      ...
      final app = await PartnerService.createDraft(accountId: accountId, customerProfileId: profileId, partnerType: PartnerType.printing, contactName: customer?['displayName']?.toString() ?? AppState.instance.displayName, mobileE164: phone, email: (customer?['email'] ?? user.email)?.toString());
  ...
  void _hydrateApplication(PartnerApplication app) {
    ...
    _hydrateBusiness(app); _hydrate(PrintingPartnerDetails.fromOnboardingData(app.onboardingData));
  }

  void _hydrateBusiness(PartnerApplication app) {
    final extensions = app.onboardingData['extensions']; if (extensions is! Map) return;
    final category = extensions['printing']; if (category is! Map) return;
  ...
  void _hydrate(PrintingPartnerDetails value) {
    ...
    _averageTurnaroundHours.text = value.averageTurnaroundHours?.toString() ?? '';
    _expressOrdersAvailable = value.expressOrdersAvailable;
  }

  int? _int(TextEditingController c) => int.tryParse(c.text.trim());
  List<String> _csv(String value) => value.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toSet().toList(growable: false);

  PrintingPartnerDetails _details() => PrintingPartnerDetails(capabilitySelection: _capabilities, serviceArea: _serviceArea.text, teamSize: _int(_teamSize), normalDailyCapacity: _int(_normalCapacity), peakDailyCapacity: _int(_peakCapacity), minimumOrderQuantity: _int(_minimumOrderQuantity), averageTurnaroundHours: _int(_averageTurnaroundHours), expressOrdersAvailable: _expressOrdersAvailable, additionalNotes: _notes.text);
  ...
  Future<void> _save({required bool submit}) async {
    ...
    try {
      ...
      await PartnerService.updatePartnerBusinessDetails(applicationId: app.id, accountId: accountId, customerProfileId: profileId, partnerType: PartnerType.printing, workshopDetails: _businessDetails());
      stage = 'Printing Partner capabilities and operations';
      await PartnerService.updatePrintingDetails(applicationId: app.id, accountId: accountId, customerProfileId: profileId, printingDetails: _details());
      ...
      if (!mounted) return; setState(() => _saving = false); _message(submit ? 'Printing Partner application submitted for Admin review.' : 'Printing Partner draft saved successfully.', false); if (submit) context.pop();
  ...
  Future<void> _applyAgain() async {
    ...
    final confirmed = await showPartnerReapplyDialog(context: context, partnerLabel: 'Printing Partner'); if (!confirmed || !mounted) return;
    setState(() => _saving = true);
    try { final app = await PartnerService.reapplyFromRejected(rejectedApplicationId: rejected.id, accountId: accountId, customerProfileId: profileId); if (!mounted) return; _hydrateApplication(app); setState(() => _saving = false); _message('A new Printing Partner Draft was created. Review the copied information before submitting.', false); } catch (error) { if (!mounted) return; setState(() => _saving = false); _message('Unable to create a new application.\n$error', true); }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null) return Scaffold(appBar: AppBar(title: const Text('Printing Partner Application')), body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Text(_error!, textAlign: TextAlign.center))));
    return Scaffold(backgroundColor: AppColors.background, appBar: AppBar(title: const Text('Printing Partner Application'), centerTitle: true, elevation: 0), body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(16), children: [
      PartnerApplicationStatusNotice(application: _application!, partnerLabel: 'Printing Partner'), const SizedBox(height: 14),
      _card('Common Partner Details', 'Common identity and contact information.', [PartnerBasicDetailsSection(contactNameController: _contact, businessNameController: _business, mobileController: _mobile, emailController: _email, editable: _editable, description: 'Common Partner identity and contact information.', businessNameLabel: 'Printing business name')]), const SizedBox(height: 14),
      _card('Business / Service Address', 'Provide the primary address and service area.', [AddressFormSection(key: ValueKey('printing-address-${_address.stateCode ?? _address.stateName ?? 'none'}-${_address.cityCode ?? _address.cityName ?? 'none'}'), metadataProvider: IndiaAddressMetadata.instance, initialValue: _address, enabled: _editable, sectionTitle: 'Business Address', addressLine1Label: 'Address', addressLine2Label: 'Address line 2', localityLabel: 'Locality or area', stateLabel: 'State', cityLabel: 'City', pincodeLabel: 'Pincode', addressLine1Required: true, addressLine2Required: false, localityRequired: false, landmarkRequired: false, stateRequired: true, cityRequired: true, pincodeRequired: true, showAddressLine2: true, showLocality: true, showLandmark: false, onChanged: (value) => _address = value), const SizedBox(height: 14), _field('Service-area pincodes', _serviceArea)]), const SizedBox(height: 14),
      _card('Operating Schedule', 'Select normal operating days and times.', [OperatingScheduleField(key: ValueKey('printing-schedule-${_schedule.normalizedOperatingDays.join('-')}-${_schedule.openingTime ?? 'none'}-${_schedule.closingTime ?? 'none'}'), initialValue: _schedule, enabled: _editable, operatingDaysRequired: true, openingTimeRequired: true, closingTimeRequired: true, requireClosingAfterOpening: true, sectionTitle: 'Operating Schedule', operatingDaysLabel: 'Operating days', openingTimeLabel: 'Opening time', closingTimeLabel: 'Closing time', onChanged: (value) => _schedule = value)]), const SizedBox(height: 14),
      _card('Printing Profile', 'Select printing services, material compatibility and fulfillment capabilities.', [CapabilityMultiSelector(metadataProvider: PrintingPartnerCapabilityMetadata.instance, partnerCategoryCode: PrintingPartnerCapabilityMetadata.categoryCode, initialValue: _capabilities, enabled: _editable, minimumSelectionCount: 0, requireOtherExpertiseDescription: false, sectionTitle: 'Printing Partner capabilities', onChanged: (value) => setState(() => _capabilities = value)), SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, value: _expressOrdersAvailable, onChanged: _editable ? (value) => setState(() => _expressOrdersAvailable = value) : null, title: const Text('Express orders available')),]), const SizedBox(height: 14),
      _card('Operations & Capacity', 'Provide capacity and operational information.', [_field('Team size', _teamSize, type: TextInputType.number), _field('Normal daily capacity', _normalCapacity, type: TextInputType.number), _field('Peak daily capacity', _peakCapacity, type: TextInputType.number), _field('Minimum order quantity', _minimumOrderQuantity, type: TextInputType.number), _field('Average turnaround time (hours)', _averageTurnaroundHours, type: TextInputType.number), _field('Additional notes', _notes, lines: 4)]), const SizedBox(height: 18), _actions(), const SizedBox(height: 24)
    ])));
  }
}
