...
function fakeAdmin() {
  ...
  const bucket = {
    file() {
      throw new Error("Storage should not be accessed for ignored events.");
    },
  };

  const db = {
    doc() {
      throw new Error("Firestore should not be accessed for ignored events.");
    },

    async runTransaction() {
      throw new Error("Transaction should not be reached for ignored events.");
    },
  };
...
test("freezes the confirmed bucket and region", () => {
  assert.equal(TARGET_BUCKET, "suisakhitest.firebasestorage.app");

  assert.equal(TARGET_REGION, "us-east1");
});
...
test("creates adapters for the confirmed Firebase resources", () => {
  ...
  assert.deepEqual(fake.calls.bucketNames, [TARGET_BUCKET]);

  assert.equal(fake.calls.firestoreCalls, 1);
});
